...
4. Provide the data quality rule in the sections mentioned "TO BE MODIFIED BY DEV"
"""
...
if __name__ == '__main__':
    ...
    try:
        ...
        evaluate_dataquality_ruleset = """
            Rules = [
                ...
                (IsComplete "CLAM_DELR_CD"),
                (IsComplete "CLAM_FOR_CD") ,
                (IsComplete "CLAM_OUTLET_CD"),
                (IsComplete "CLAM_DUP_SL_NO") ,
                (IsComplete "CLAM_ISSUE_NO"),
        ...
        (consolidated_dq_result_dyf, final_dq_status) = after_processing_generate_dq_overall_status_consolidated(
            glueContext, spark,
    ...
    try:
        ...
        ########################################################################################################################
        ### Code for validating if DQ rule passed or failed 
        # data_check_df = consolidated_dq_result_dyf.toDF()
        # data_check_df.createOrReplaceTempView("data_check_df")
        # final_dq_result_df = spark.sql(f"""select outcome FROM data_check_df where rule = '{dq_overall_status_rule_name}' """)
        # for row in final_dq_result_df.rdd.collect():
        #     final_dq_status = row['outcome']

        # log.info(f"Final DataQuality Result for the {source_table} is {final_dq_status} in the workflow id {corelation_id}")
